/*
Package auth wraps common behaviour when using GoAuth. This makes it quick, and easy, to get up
and running with GoAuth. Of course, if you want complete control over how things flow, in regard
to the authentication process, feel free and use GoAuth directly.

See https://github.com/izetmolla/goauth/blob/master/examples/main.go to see this in action.
*/
package auth
...
const SessionName = "_goauth_session"
...
func GetAuthURL(res http.ResponseWriter, req *http.Request) (string, error) {
	...
	provider, err := goauth.GetProvider(providerName)
	if err != nil {
		return "", err
	}
...
See https://github.com/izetmolla/goauth/blob/master/examples/main.go to see this in action.
*/
var CompleteUserAuth = func(res http.ResponseWriter, req *http.Request) (goauth.User, error) {
	if !keySet && defaultStore == Store {
		fmt.Println("goth/gothic: no SESSION_SECRET environment variable is set. The default cookie store is not available and any calls will fail. Ignore this warning if you are using a different store.")
	}
...
func validateState(req *http.Request, sess goauth.Session) error {
