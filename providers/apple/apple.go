...
// This package can be used as a reference implementation of an OAuth2 provider for GoAuth.
package apple
...
func (p Provider) BeginAuth(state string) (goauth.Session, error) {
...
func (Provider) UnmarshalSession(data string) (goauth.Session, error) {
	s := &Session{}
...
func (p Provider) FetchUser(session goauth.Session) (goauth.User, error) {
	s := session.(*Session)
	if s.AccessToken == "" {
		return goauth.User{}, fmt.Errorf("no access token obtained for session with provider %s", p.Name())
	}
	return goauth.User{
		Provider:     p.Name(),
		UserID:       s.ID.Sub,
...
func (p Provider) Client() *http.Client {
	return goauth.HTTPClientWithFallBack(p.httpClient)
}

func (p Provider) RefreshToken(refreshToken string) (*oauth2.Token, error) {
	token := &oauth2.Token{RefreshToken: refreshToken}
	ts := p.config.TokenSource(goauth.ContextForClient(p.Client()), token)
